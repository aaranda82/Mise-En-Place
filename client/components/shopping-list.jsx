    ...
    fetch('/api/shoppinglist', init)
      ...
      .then(shoppingList => {
        if (!shoppingList.error) {
          ...
          this.setState({
            shoppingList: [],
            errorMessage: shoppingList.error
          });
          Swal.fire(this.state.errorMessage);
        }
      });
  }
...
function Item(props) {
  const details = props.ingredient.recipe.map((element, index) => {
    ...
    return <p className="text-left m-0" key={index}>{`${element.quantity} ${unit} for ${element.recipeName}`}</p>;
  });
